...
class LivewireDatatableQueryBuilderTest extends TestCase
{
    ...
    public function it_creates_a_where_query_for_belongs_to_relation_columns()
    {
        ...
        $this->assertEquals('select "dummy_has_many_models"."id" as "id", "dummy_models"."subject" as "dummy_model.subject" from "dummy_has_many_models" left join "dummy_models" on "dummy_models"."id" = "dummy_has_many_models"."dummy_model_id" where (dummy_models.subject >= ?) order by dummy_has_many_models.id desc', $subject->getQuery()->toSql());
        ...
        $subject->forgetComputed();
        $this->assertEquals('select "dummy_has_many_models"."id" as "id", "dummy_models"."subject" as "dummy_model.subject" from "dummy_has_many_models" left join "dummy_models" on "dummy_models"."id" = "dummy_has_many_models"."dummy_model_id" order by dummy_has_many_models.id desc', $subject->getQuery()->toSql());
